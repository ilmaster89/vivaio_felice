...
public class DipendenteJdbcDao {
	...
	public List<Dipendente> login(String user_name, String password) {
		...
		return jdbcTemplate.query(
				"select * from dipendenti join livelli on dipendenti.livello_id = livelli.id where user_name = ? and password = ?",
				new Object[] { user_name, password },
				(rs, rowNum) -> new Dipendente(rs.getInt("dipendenti.id"),
						new Livello(rs.getInt("livello_id"), rs.getString("mansione")), rs.getString("nome"),
